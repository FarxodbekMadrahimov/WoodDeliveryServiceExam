    ...
    public class TokenService : ITokenService
    {
        ...
        public string GenerateJWT(IEnumerable<Claim> additionalClaims = null)
        {
            ...
            var claims = new List<Claim>
            {
                ...
                //new Claim(JwtRegisteredClaimNames.Iat, DateTimeOffset.itcNow.ToString()), .NET 6 da bu //new Claim(JwtRegistered ClaimNames.Iat, DateTimeOffset.UtcNow.ToUnixTimeSeconds().ToString()) .NET 7 da bu
                new Claim(JwtRegisteredClaimNames.Iat, EpochTime.GetIntDate(DateTime.UtcNow).ToString(CultureInfo.InvariantCulture),
                 ClaimValueTypes. Integer64) // .NET 8 da shunaqa bo'ladikan yangilanibdi) shu 1 qator codent deb ishlamayato dikan
            };
        ...
        public async ValueTask<TokenDTO> RefreshToken(RefreshTokenDTO refreshTokenDTO)
        {
            ...
            var user = await _context.UserAdmin.FirstOrDefaultAsync(x => x.Id == id);
            ...
            return new TokenDTO()
            {
                ...
                ExpireDate = user.RefreshTokenExpireDate ?? DateTime.Now.AddMinutes(5)
            };
        }
    }
}